...
<div class="row justify-content-center mt-4">
    <div class="col-md-10">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('restaurants.update', $restaurant) }}" id="restaurantForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('timezone') is-invalid @enderror"
                            ...
                            <option value="Asia/Jakarta" {{ old('timezone', $restaurant->timezone) == 'Asia/Jakarta' ? 'selected' : '' }}>WIB - Jakarta (UTC+7)</option>
                            <option value="Asia/Makassar" {{ old('timezone', $restaurant->timezone) == 'Asia/Makassar' ? 'selected' : '' }}>WITA - Makassar (UTC+8)</option>
                            <option value="Asia/Jayapura" {{ old('timezone', $restaurant->timezone) == 'Asia/Jayapura' ? 'selected' : '' }}>WIT - Jayapura (UTC+9)</option>
                            <option value="Asia/Pontianak" {{ old('timezone', $restaurant->timezone) == 'Asia/Pontianak' ? 'selected' : '' }}>WIB - Pontianak (UTC+7)</option>
                        </select>
                        @error('timezone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror"
                                  ...
                                  placeholder="Describe your restaurant, specialties, atmosphere, etc.">{{ old('description', $restaurant->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="is_inside_mall" name="is_inside_mall" value="1"
                                   {{ old('is_inside_mall', $restaurant->is_inside_mall) ? 'checked' : '' }}>
                    ...
                    <div class="mall-fields mb-3">
                        ...
                        <input type="text" class="form-control @error('mall_name') is-invalid @enderror"
                               id="mall_name" name="mall_name" value="{{ old('mall_name', $restaurant->mall_name) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize map
    const defaultLat = {{ $restaurant->latitude ?? -6.2088 }}; // Jakarta coordinates or existing
    const defaultLng = {{ $restaurant->longitude ?? 106.8456 }};
    ...
    if ({{ $restaurant->latitude ? 'true' : 'false' }} && {{ $restaurant->longitude ? 'true' : 'false' }}) {
